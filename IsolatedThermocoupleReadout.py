...
class IsolatedThermocoupleReadout:
    def __init__(self, time_offset, resource_name, channels, units = 'K'):
        ...
        try:
            ...
            logging.warning("IsolatedThermocoupleReadout connection error1: "+str(err))
        ...
        try:
            ...
            logging.warning("IsolatedThermocoupleReadout connection error2: "+str(err))
    ...
    def GetTemperature(self, ch):
        temperature = float((self.instr.query(ch)).rpartition("s")[2])
        if self.units == 'K':
